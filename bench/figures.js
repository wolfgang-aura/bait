...
const RECORDED = {
  ...
  video: {
    file: 'scratch/BAIT-judge-v28.mp4',
    seconds: 44.8,
    fps: 30,
    width: 1920,
    height: 1080,
    sha256: '708298a00dca105a92ab5c608472a20e7840973bc4fe395dfdee30efa9927ae2',
...
export async function computeFigures() {
  ...
  // 9. The field test (bench/FIELD.md): the shipped gate on the top of the 30-day leaderboard, from bench/field/reads.
  const field = await (await import('./field.js')).fieldFigures();

  return {
    about: 'Canonical headline figures. Generated by `node bench/figures.js --write` from the committed rows, reports and raw reads; bench/figures.test.js fails if this file drifts from them or any doc or served page drifts from this file. Every [n, d] is n of d; docs/EVIDENCE.md "The numbers" explains each denominator.',
...
function phraseRules(F) {
  ...
  return [
    ...
    ...(F.field ? [{ name: 'field headline', re: /\b[Oo]f (\d+) top leaderboard wallets, (\d+|none)\b/g, want: [F.field.scored, F.field.operator.flagged], map: w => (w === 'none' ? 0 : n(w)) }] : []),
