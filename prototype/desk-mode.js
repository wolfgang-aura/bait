 ...
 */
export function validatePennyTurn(body, { roomDesk, formatSuffix, maxTurns = 7, maxChars = 1500 }) {
  ...
  for (const t of turns) {
    if (!['user', 'assistant'].includes(t?.role) || typeof t.content !== 'string' || t.content.length > maxChars) return { ok: false, error: 'bad turns' };
  }
  return { ok: true, system, turns: turns.map(t => ({ role: t.role, content: t.content })) };
}
