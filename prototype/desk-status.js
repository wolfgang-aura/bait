...
export const DESK_MESSAGES = Object.freeze({
  no_key: 'No DEEPSEEK_API_KEY in .env, so PENNY cannot answer. Playing a round needs that key; npm run bench needs no keys.',
  hosted_cap: 'Today’s live rounds are used up. The recorded proof is still open.',
  local_cap: 'PENNY’s model-call budget on this machine is spent, so it cannot answer. npm run bench needs no keys.',
});
